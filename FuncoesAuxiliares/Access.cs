    ...
    public static class Access
    {
        ...
        public static int ExecuteScalar(IDbCommand cmd, bool registrolog = true)
        {
            ...
            do
            {
                ...
                try
                {
                    foreach (NpgsqlParameter parmt in cmd.Parameters)
                        try
                        {
                ...
                catch (NpgsqlException ex)
                {
                    ...
                    if (ex.Message.Contains("Exception while writing to stream") || ex.Message.Contains("Exception while reading from stream"))
                    {
                        ...
                        TrataExcecao(ex, (NpgsqlCommand)cmd);
        ...
        {
            string connectionString = @"Server=localhost;Port=5432;Database=agendaPrime;User Id=sayajin;Password=Sh1ft!@#;";
        ...
        public static DataSet ExecuteReader(IDbCommand command)
        {
            ...
            do
            {
                ...
                try
                {
                    foreach (NpgsqlParameter parmt in command.Parameters)
                        try
                        {
                            if (parmt.Value == null || parmt.Value.ToString() == char.MinValue.ToString())
                                parmt.Value = DBNull.Value;
                    ...
                    IDbDataAdapter da = new NpgsqlDataAdapter((NpgsqlCommand)command);
                    ds = new DataSet();
                ...
                catch (NpgsqlException ex)
                {
                    ...
                    if (ex.Message.Contains("Exception while writing to stream") || ex.Message.Contains("Exception while reading from stream"))
                    {
                        ...
                        TrataExcecao(ex, (NpgsqlCommand)command);
                }
        ...
        public static NpgsqlDataReader ReaderExecute(NpgsqlCommand command)
        {
            bool retry = false;
            int tentativa = 1;
            ...
            do
            {
                ...
                if (command.Connection == null)
                    command.Connection = (NpgsqlConnection)GetConnection();
